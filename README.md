...
当前推荐从 GitHub 克隆本仓库本地运行；暂未发布 npm 包。等真实页面适配和安装路径更稳定后，再考虑公开 npm 分发。

## 第一次安全同步

1. 在自己的 Mac 或私有服务器上运行本项目，不要暴露公网入口。
2. 用可见浏览器完成淘宝、京东或菜鸟扫码登录。
3. 调用 `sync_packages`，把包裹摘要同步到本地 JSON。
4. 在 AI 客户端里问：“今天有哪些包裹要到？”
